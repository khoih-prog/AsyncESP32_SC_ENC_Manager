...
* [Why do we need this AsyncESP32_SC_ENC_Manager library](#why-do-we-need-this-async-AsyncESP32_SC_ENC_Manager-library)
  * [Features](#features)
  * [Why Async is better](#why-async-is-better)
  * [Sibling Libraries](#Similar-Libraries)
...
* [Debug Terminal Output Samples](#debug-terminal-output-samples)
  * [1. Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60](#1-Async_ConfigOnDoubleReset_TZ-using-LittleFS-on-ESP32S3_DEV-with-ESP32_S3_ENC28J60)
  * [2. Async_ConfigOnSwichFS using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60](#2-Async_ConfigOnSwichFS-using-LittleFS-on-ESP32S3_DEV-with-ESP32_S3_ENC28J60)
  * [3. Async_ESP32_FSWebServer_DRD using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60](#3-Async_ESP32_FSWebServer_DRD-using-LittleFS-on-ESP32S3_DEV-with-ESP32_S3_ENC28J60)
  * [4. Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32C3_DEV with ESP32_C3_ENC28J60](#4-Async_ConfigOnDoubleReset_TZ-using-LittleFS-on-ESP32C3_DEV-with-ESP32_C3_ENC28J60)
  * [5. Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32S2_DEV with ESP32_S2_ENC28J60](#5-Async_ConfigOnDoubleReset_TZ-using-LittleFS-on-ESP32S2_DEV-with-ESP32_S2_ENC28J60)
* [Debug](#debug)
...
### Why do we need this Async [AsyncESP32_SC_ENC_Manager library](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager)
...
to use the better and faster **asynchronous** [ESPAsyncWebServer](https://github.com/me-no-dev/ESPAsyncWebServer) instead of [WebServer_ESP32_ENC]((https://github.com/khoih-prog/WebServer_ESP32_ENC))

Thanks to this [AsyncESP32_SC_ENC_Manager library](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager) is based on and sync'ed with [`ESPAsync_WiFiManager`](https://github.com/khoih-prog/ESPAsync_WiFiManager), all the features currently supported by [`ESPAsync_WiFiManager`](https://github.com/khoih-prog/ESPAsync_WiFiManager) will be available. Please have a look at [`ESPAsync_WiFiManager`](https://github.com/khoih-prog/ESPAsync_WiFiManager) for those too-many-to-list features.
...
This [**AsyncESP32_SC_ENC_Manager** library](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager) currently supports these following boards:
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/ESP32S3_DEV.png">
</p>
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/ESP32S2_DEV.png">
</p> 
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/ESP32_C3_DevKitC_02.png">
</p> 
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/ENC28J60.png">
</p>
...
The best and easiest way is to use `Arduino Library Manager`. Search for `AsyncESP32_SC_ENC_Manager`, then select / install the latest version. You can also use this link [![arduino-library-badge](https://www.ardu-badge.com/badge/AsyncESP32_SC_ENC_Manager.svg?)](https://www.ardu-badge.com/AsyncESP32_SC_ENC_Manager) for more detailed instructions.

### Manual Install

1. Navigate to [AsyncESP32_SC_ENC_Manager](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager) page.
2. Download the latest release `AsyncESP32_SC_ENC_Manager-main.zip`.
3. Extract the zip file to `AsyncESP32_SC_ENC_Manager-main` directory 
4. Copy the whole `AsyncESP32_SC_ENC_Manager-main` folder to Arduino libraries' directory such as `~/Arduino/libraries/`.

### VS Code & PlatformIO:

1. Install [VS Code](https://code.visualstudio.com/)
2. Install [PlatformIO](https://platformio.org/platformio-ide)
3. Install [**AsyncESP32_SC_ENC_Manager** library](https://registry.platformio.org/libraries/khoih-prog/AsyncESP32_SC_ENC_Manager) by using [Library Manager](https://registry.platformio.org/libraries/khoih-prog/AsyncESP32_SC_ENC_Manager/installation). Search for **AsyncESP32_SC_ENC_Manager** in [Platform.io Author's Libraries](https://platformio.org/lib/search?query=author:%22Khoi%20Hoang%22)
...
```cpp
#include <AsyncESP32_SC_ENC_Manager.hpp>               //https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager
```

in many files. But be sure to use the following `#include <AsyncESP32_SC_ENC_Manager.h>` **in just 1 `.h`, `.cpp` or `.ino` file**, which must **not be included in any other file**, to avoid `Multiple Definitions` Linker Error

```cpp
// To be included only in main(), .ino with setup() to avoid `Multiple Definitions` Linker Error
#include <AsyncESP32_SC_ENC_Manager.h>          //https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager
```
...
// Must be placed before #include <AsyncESP32_SC_ENC_Manager.h>
...
///////////////////////////////////////////
/******************************************
   // Defined in AsyncESP32_SC_ENC_Manager.hpp
  typedef struct
  {
...
```cpp
//AsyncESP32_SC_ENC_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask, dns1IP, dns2IP);
AsyncESP32_SC_ENC_manager.setSTAStaticIPConfig(WM_STA_IPconfig);
```
...
```cpp
String tempTZ = AsyncESP32_SC_ENC_manager.getTimezoneName();
```
...
```cpp
const char * TZ_Result = AsyncESP32_SC_ENC_manager.getTZ(_timezoneName);
```
...
AsyncESP32_SC_ENC_Manager AsyncESP32_SC_ENC_manager(&webServer, &dnsServer);
```
...
```cpp
AsyncESP32_SC_ENC_Manager AsyncESP32_SC_ENC_manager(&webServer, &dnsServer, "Personalized-HostName");
```

then later call

```cpp
AsyncESP32_SC_ENC_manager.startConfigPortal()
```
...
These illustrating steps is based on the example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/tree/main/examples/Async_ConfigOnSwitchFS)
...
The example [Async_ConfigOnSwitchFS](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/tree/main/examples/Async_ConfigOnSwitchFS) will open the CP whenever a SW press is detected in `loop()`. So the code to add `dynamic variables` will be there, just after the CP `AsyncESP32_SC_ENC_Manager` class initialization to create `AsyncESP32_SC_ENC_Manager` object.
...
// is configuration portal requested?
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    AsyncESP32_SC_ENC_Manager AsyncESP32_SC_ENC_manager(&webServer, &dnsServer, "ConfigOnSwitchFS");
...
Adding those `ESPAsync_EMParameter` objects created in Step 2 using the function `addParameter()` of object `AsyncESP32_SC_ENC_Manager`
...
AsyncESP32_SC_ENC_manager.addParameter(&p_thingspeakApiKey);
AsyncESP32_SC_ENC_manager.addParameter(&p_sensorDht22);
AsyncESP32_SC_ENC_manager.addParameter(&p_pinSda);
AsyncESP32_SC_ENC_manager.addParameter(&p_pinScl);
```
...
After users select `Save`, the CP `AsyncESP32_SC_ENC_Manager` object will save the user input data into related `ESPAsync_EMParameter` objects.
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/Main.png">
</p>

Select `Information` to enter the Info page where the board info will be shown (long page)

<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/Info.png">
</p>
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/Configuration_Standard.png">
</p>
...
<p align="center">
    <img src="https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/raw/main/Images/Saved.png">
</p>
...
```cpp
AsyncESP32_SC_ENC_manager.setSaveConfigCallback(saveConfigCallback);
```
...
void loop()
{
  ...
  if ((digitalRead(TRIGGER_PIN) == LOW) || (digitalRead(TRIGGER_PIN2) == LOW))
  {
    ...
    AsyncESP32_SC_ENC_Manager AsyncESP32_SC_ENC_manager(&webServer, NULL, "ConfigOnSwitch");
#else
    AsyncDNSServer dnsServer;

    AsyncESP32_SC_ENC_Manager AsyncESP32_SC_ENC_manager(&webServer, &dnsServer, "ConfigOnSwitch");
#endif
    ...
    AsyncESP32_SC_ENC_manager.setSTAStaticIPConfig(stationIP, gatewayIP, netMask);
#endif
#endif
    ...
    else
    {
      ...
      AsyncESP32_SC_ENC_manager.setConfigPortalTimeout(0);
    ...
    if (!AsyncESP32_SC_ENC_manager.startConfigPortal())
    ...
    const char * TZ_Result = AsyncESP32_SC_ENC_manager.getTZ(Ethconfig.TZ_Name);
    ...
    AsyncESP32_SC_ENC_manager.getSTAStaticIPConfig(EthSTA_IPconfig);
...
Many applications need configuration parameters like `MQTT host and port`, [Blynk](http://www.blynk.cc) or [emoncms](http://emoncms.org) tokens, etc. While it is possible to use [`AsyncESP32_SC_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager) to collect additional parameters, it is better to read these parameters from a web service once [`AsyncESP32_SC_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager) has been used to connect to the Internet.

To capture other parameters with [`AsyncESP32_SC_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager) is a little bit more complicated than all the other features. This requires adding custom HTML to your form. 

If you want to do it with [`AsyncESP32_SC_ENC_Manager`](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager) see the example [Async_ConfigOnSwitchFS](examples/Async_ConfigOnSwitchFS)
...
```cpp
AsyncESP32_SC_ENC_manager.setSTAStaticIPConfig(IPAddress(192,168,2,232), IPAddress(192,168,2,1), IPAddress(255,255,255,0));
```
...
```cpp
AsyncESP32_SC_ENC_manager.setCustomHeadElement("<style>html{filter: invert(100%); -webkit-filter: invert(100%);}</style>");
```
...
AsyncESP32_SC_ENC_manager.addParameter(&custom_text);
```
...
https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/blob/63f09a82af2f5ed1da9e88879b14facb1d94ea88/examples/Async_ConfigOnSwitch/Async_ConfigOnSwitch.ino#L28-L935
...
#### 1. Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60

##### 1.1 DRD => Config Portal

This is terminal debug output when running [Async_ConfigOnDoubleReset_TZ](examples/Async_ConfigOnDoubleReset_TZ) on  **ESP32S3_DEV + LwIP ENC28J60**. `Config Portal` was requested by DRD to input and save Credentials, such as Static IP address.

```
Starting Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60
AsyncESP32_SC_ENC_Manager v1.2.0
...
Starting configuration portal @ 192.168.2.105
[EM] _configPortalStart millis() = 2565
[EM] Config Portal IP address = 192.168.2.105
[EM] HTTP server started
[EM] startConfigPortal : Enter loop
[EM] handleRoot
[EM] request host IP = 192.168.2.105
[EM] request host IP = 192.168.2.105
[EM] Info
[EM] Info page sent
[EM] handleRoot
...
[EM] OK
After waiting 0.00 secs more in setup(), connection result is connected. Local IP: 192.168.2.105
[EM] freeing allocated params!
Stop doubleResetDetecting
Saving config file...
Saving config file OK
Local Date/Time: Fri Dec 23 19:15:28 2022
Local Date/Time: Fri Dec 23 19:16:28 2022
```

---

#### 2. Async_ConfigOnSwichFS using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60

This is terminal debug output when running [Async_ConfigOnSwichFS](examples/Async_ConfigOnSwichFS) on  **ESP32S3_DEV + LwIP ENC28J60**. `Config Portal` was requested to input and save Credentials.


```
Starting Async_ConfigOnSwichFS using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60
AsyncESP32_SC_ENC_Manager v1.2.0
[EM] Default SPI pinout:
[EM] MOSI: 11
[EM] MISO: 13
...
[EM] freeing allocated params!
Local Date/Time: Fri Dec 23 19:16:32 2022
...
Config file was successfully saved
[EM] freeing allocated params!
Local Date/Time: Fri Dec 23 19:17:28 2022
Local Date/Time: Fri Dec 23 19:18:28 2022
```

---

#### 3. Async_ESP32_FSWebServer_DRD using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60

This is terminal debug output when running [Async_ESP32_FSWebServer_DRD](examples/Async_ESP32_FSWebServer_DRD)  on  **ESP32S3_DEV + LwIP ENC28J60**. `Config Portal` was requested by DRD (also using **LittleFS**) to input and save Credentials.


```cpp
Starting Async_ESP32_FSWebServer_DRD using LittleFS on ESP32S3_DEV with ESP32_S3_ENC28J60
AsyncESP32_SC_ENC_Manager v1.2.0
ESP_DoubleResetDetector v1.3.2
...
[EM] dns1IP = 192.168.2.1 , dns2IP = 8.8.8.8
ETH Connected
ETH MAC: DE:AD:BE:EF:BE:06, IPv4: 192.168.2.233
FULL_DUPLEX, 10Mbps
Got stored Credentials. Timeout 120s for Config Portal
LittleFS Flag read = 0xD0D04321
No doubleResetDetected
...
===============================================================
[EM] freeing allocated params!
HNOT_FOUND: GET http://192.168.2.233/edit
_HEADER[Host]: 192.168.2.233
_HEADER[Connection]: keep-alive
_HEADER[Authorization]: Digest username="admin", realm="asyncesp", nonce="795d0e2e77f0bd1ac56d88a223e30cc8", uri="/edit?edit=CanadaFlag_1.png&_cb=1671066434775", response="e73c54b485c1237f0fcb38f42266dbdb", opaque="0bc073c1bf61fa0ae678fa5892cfd2a6", qop=auth, nc=0000001b, cnonce="0dc7bfe4a8255a4d"
...
_HEADER[Referer]: http://192.168.2.233/edit
...
#### 4. Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32C3_DEV with ESP32_C3_ENC28J60

This is terminal debug output when running [Async_ConfigOnDoubleReset_TZ](examples/Async_ConfigOnDoubleReset_TZ) on  **ESP32C3_DEV + LwIP ENC28J60**

```
Starting Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32C3_DEV with ESP32_C3_ENC28J60
AsyncESP32_SC_ENC_Manager v1.1.0
ESP_DoubleResetDetector v1.3.2
[EM] Default SPI pinout:
[EM] SPI_HOST: 1
[EM] MOSI: 6
...
After waiting 0.00 secs more in setup(), connection result is connected. Local IP: 192.168.2.232
[EM] freeing allocated params!
Local Date/Time: Fri Dec 23 19:12:30 2022
Stop doubleResetDetecting
Saving config file...
Saving config file OK
Local Date/Time: Fri Dec 23 19:13:28 2022
Local Date/Time: Fri Dec 23 19:14:28 2022
```

---

#### 5. Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32S2_DEV with ESP32_S2_ENC28J60

This is terminal debug output when running [Async_ConfigOnDoubleReset_TZ](examples/Async_ConfigOnDoubleReset_TZ) on  **ESP32C3_DEV + LwIP ENC28J60**

```
Starting Async_ConfigOnDoubleReset_TZ using LittleFS on ESP32S2_DEV with ESP32_S2_ENC28J60
AsyncESP32_SC_ENC_Manager v1.1.0
ESP_DoubleResetDetector v1.3.2
[EM] Default SPI pinout:
[EM] SPI_HOST: 1
...
After waiting 0.00 secs more in setup(), connection result is connected. Local IP: 192.168.2.232
[EM] freeing allocated params!
Local Date/Time: Fri Dec 23 19:25:17 2022
Local Date/Time: Fri Dec 23 19:26:17 2022
```
...
```cpp
AsyncESP32_SC_ENC_manager.setDebugOutput(false);
```
...
Submit issues to: [AsyncESP32_SC_ENC_Manager issues](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/issues)
...
### DONE
 ...
 1. Add support to **ESP32/S3-based boards using LwIP ENC28J60 Ethernet**
 2. Add debugging features
 3. Add astyle using `allman` style. Restyle the library
 4. Add support to **ESP32_S2/C3-based boards using LwIP ENC28J60 Ethernet**
...
- The library is licensed under [MIT](https://github.com/khoih-prog/AsyncESP32_SC_ENC_Manager/blob/main/LICENSE)
